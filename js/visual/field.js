// 52-week per year life canvas renderer (High-contrast compact square matrix)
...
export class LifeMatrixRenderer {
  ...
  render(snapshot) {
    ...
    // 1. Clear, readable decade labels on left (11px)
    this.ctx.font = "11px -apple-system, BlinkMacSystemFont, 'SF Pro Text', sans-serif";
    ...
    for (let r = 0; r <= rows; r += 10) {
      ...
      this.ctx.fillText(`${r}`, startX - 8, y);
    }

    // 2. Future unlived weeks (clearly visible subtle points)
    ...
    // 3. Past lived weeks (high-contrast white/silver solid texture)
    ...
    // 4. Active Present Week (Glowing white beacon)
